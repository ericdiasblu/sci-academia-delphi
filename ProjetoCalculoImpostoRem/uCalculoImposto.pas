...
type
  TfrCalculoImposto = class(TForm)
    ...
    DBGrid1: TDBGrid;
    btCalcularImpostosEstado: TButton;
    btDemonstracao: TButton;
    ...
    procedure btDemonstracaoClick(Sender: TObject);
  private
...
procedure TfrCalculoImposto.btDemonstracaoClick(Sender: TObject);
var
  wParana: Currency;
  wSantaCatarina: Currency;
  wSaoPaulo: Currency;
begin
  wParana := 0;
  wSantaCatarina := 0;
  ...
  cdsCalculoImposto.First;

  while not cdsCalculoImposto.Eof do
    begin
        if cdsCalculoImpostobdESTADO.AsInteger = 0 then
          begin
            wParana := wParana + cdsCalculoImpostobdIMPOSTO.AsCurrency;
          end
        else if cdsCalculoImpostobdESTADO.AsInteger = 1 then
          begin
            wSantaCatarina := wSantaCatarina + cdsCalculoImpostobdIMPOSTO.AsCurrency;
          end
        else if cdsCalculoImpostobdESTADO.AsInteger = 2 then
          begin
            wSaoPaulo := wSaoPaulo + cdsCalculoImpostobdIMPOSTO.AsCurrency;
          end;

          cdsCalculoImposto.Next;
    end;

    ShowMessage('Paraná: '+CurrToStr(wParana) + #13 +
                'Santa Catarina: '+CurrToStr(wSantaCatarina) + #13 +
                'São Paulo: '+CurrToStr(wSaoPaulo) + #13);
